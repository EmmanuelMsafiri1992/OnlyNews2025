...
<body>
        ...
        <div class="row">
            ...
            <div class="col-md-10 main-content p-4">
                ...
                <ul class="nav nav-tabs mb-4" id="settingsTab" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="network-tab" data-bs-toggle="tab" data-bs-target="#network"
                            type="button" role="tab" aria-controls="network"
                            aria-selected="false">{{ __('Network') }}</button>
                    </li>
                    @endif
                </ul>
                ...
                <div class="tab-content" id="settingsTabContent">
                    ...
                    {{-- Network Settings Tab (Superadmin Only) --}}
                    @if(Auth::user()->isSuperAdmin())
                    <div class="tab-pane fade" id="network" role="tabpanel" aria-labelledby="network-tab">
                        <div class="row">
                            <div class="col-md-8 mb-4">
                                <div class="card p-3">
                                    <h5 class="mb-3">
                                        <i class="fas fa-network-wired"></i> {{ __('Network Configuration') }}
                                    </h5>
                                    <div class="alert alert-warning">
                                        <i class="fas fa-exclamation-triangle"></i>
                                        <strong>{{ __('Warning:') }}</strong> {{ __('Changing network settings may affect connectivity. Ensure you have physical access to the device.') }}
                                    </div>

                                    <form action="{{ route('admin.settings.network') }}" method="POST" id="networkForm">
                                        @csrf

                                        <div class="mb-3">
                                            <label class="form-label">{{ __('Network Mode') }}</label>
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="mode" id="dhcp" value="dhcp"
                                                    {{ ($settings['network_mode'] ?? 'dhcp') == 'dhcp' ? 'checked' : '' }}>
                                                <label class="form-check-label" for="dhcp">
                                                    <strong>{{ __('DHCP (Automatic)') }}</strong>
                                                    <br><small class="text-muted">{{ __('Automatically obtain IP address from router') }}</small>
                                                </label>
                                            </div>
                                            <div class="form-check mt-2">
                                                <input class="form-check-input" type="radio" name="mode" id="static" value="static"
                                                    {{ ($settings['network_mode'] ?? 'dhcp') == 'static' ? 'checked' : '' }}>
                                                <label class="form-check-label" for="static">
                                                    <strong>{{ __('Static IP') }}</strong>
                                                    <br><small class="text-muted">{{ __('Manually configure IP address') }}</small>
                                                </label>
                                            </div>
                                        </div>
                                        ...
                                        <div id="staticFields" style="display: {{ ($settings['network_mode'] ?? 'dhcp') == 'static' ? 'block' : 'none' }};">
                                            <div class="row">
                                                <div class="col-md-6 mb-3">
                                                    <label for="ip_address" class="form-label">{{ __('IP Address') }}</label>
                                                    <input type="text" class="form-control" id="ip_address" name="ip_address"
                                                        value="{{ $settings['static_ip'] ?? '' }}" placeholder="192.168.1.100">
                                                    @error('ip_address')
                                                        <div class="text-danger mt-2">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label for="subnet_mask" class="form-label">{{ __('Subnet Mask') }}</label>
                                                    <select class="form-control" id="subnet_mask" name="subnet_mask">
                                                        <option value="255.255.255.0" selected>255.255.255.0 (/24)</option>
                                            ...
                                            <div class="row">
                                                <div class="col-md-6 mb-3">
                                                    <label for="gateway" class="form-label">{{ __('Gateway') }}</label>
                                                    <input type="text" class="form-control" id="gateway" name="gateway"
                                                        value="{{ $settings['static_gateway'] ?? '' }}" placeholder="192.168.1.1">
                                                    @error('gateway')
                                                        <div class="text-danger mt-2">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label for="dns_server" class="form-label">{{ __('DNS Server') }}</label>
                                                    <input type="text" class="form-control" id="dns_server" name="dns_server"
                                                        value="{{ $settings['static_dns'] ?? '8.8.8.8' }}" placeholder="8.8.8.8">
                                                    @error('dns_server')
                                                        <div class="text-danger mt-2">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                        ...
                                        <div class="mt-4">
                                            <button type="submit" class="btn btn-primary" id="saveNetworkBtn">
                                                <i class="fas fa-save"></i> {{ __('Apply Network Settings') }}
                                            </button>
                                            <button type="button" class="btn btn-info ms-2" id="getCurrentNetworkBtn">
                                                <i class="fas fa-sync"></i> {{ __('Refresh Current Settings') }}
                                            </button>
                                        </div>
                                    </form>
                                </div>
                            ...
                            <div class="col-md-4 mb-4">
                                <div class="card p-3">
                                    <h5 class="mb-3">
                                        <i class="fas fa-info-circle"></i> {{ __('Current Network Status') }}
                                    </h5>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const getCurrentNetworkBtn = document.getElementById('getCurrentNetworkBtn');
            ...
            // Add event listeners
            if (dhcpRadio) dhcpRadio.addEventListener('change', toggleStaticFields);
            if (staticRadio) staticRadio.addEventListener('change', toggleStaticFields);

            // Load current network status
            function loadNetworkStatus() {
                if (!networkStatus) return;

                fetch('{{ route("admin.settings.network.get") }}')
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            networkStatus.innerHTML = `
                                <div class="mb-2">
                                    <strong>{{ __('Current IP:') }}</strong>
                                    <span class="badge bg-success">${network.current_ip}</span>
                                </div>
                                <div class="mb-2">
                                    <strong>{{ __('Mode:') }}</strong>
                                    <span class="badge ${network.mode === 'static' ? 'bg-warning' : 'bg-info'}">${network.mode.toUpperCase()}</span>
                                </div>
                                <div class="mb-2">
                                    <strong>{{ __('Interface:') }}</strong>
                                    <span class="text-muted">${network.interface}</span>
                                </div>
                                <div class="mb-2">
                                    <strong>{{ __('Gateway:') }}</strong>
                                    <span class="text-muted">${network.gateway || 'N/A'}</span>
                                </div>
                                <div>
                                    <strong>{{ __('DNS:') }}</strong>
                                    <span class="text-muted">${network.dns_servers ? network.dns_servers.join(', ') : 'N/A'}</span>
                                </div>
                            `;
                        } else {
                            networkStatus.innerHTML = '<div class="text-danger">{{ __("Failed to load network status") }}</div>';
                        }
                    })
                    .catch(error => {
                        console.error('Error loading network status:', error);
                        networkStatus.innerHTML = '<div class="text-danger">{{ __("Error loading network status") }}</div>';
                    });
            }
            ...
            // Refresh button
            if (getCurrentNetworkBtn) {
                getCurrentNetworkBtn.addEventListener('click', function() {
                    networkStatus.innerHTML = `
                        <div class="d-flex justify-content-center align-items-center" style="height: 100px;">
                            <div class="spinner-border text-primary" role="status">
                                <span class="visually-hidden">{{ __('Loading...') }}</span>
            ...
            if (networkForm) {
                networkForm.addEventListener('submit', function(e) {
                    const isStatic = staticRadio && staticRadio.checked;
                    const message = isStatic
                        ? '{{ __("Are you sure you want to apply static IP configuration? This may affect connectivity.") }}'
                        : '{{ __("Are you sure you want to switch to DHCP? The IP address may change.") }}';
